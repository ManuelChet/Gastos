...
function validarCamposCondicionales() {
  ...
  // Hipotecario - Siempre es Escritura Registrada    
    ...
    // ✅ Cantidad de escrituras desde Q1,000
    if (monto >= 1000) {
      cantidadEscriturasHipotecarioContainer.classList.remove('hidden');
    }

    // ✅ Ubicación solo si es mayor a Q400,000
    if (monto > 400000) {
  ...
  if (tipo === 'derechos_posesorios') {
    ...
    // Solo mostrar cantidad de escrituras y ubicacion para montos mayores a Q400,000
    if (monto > 400000) {
...
if (tipo === 'microcredito') {
...
else if (tipo === 'credito_seguro_columna') {
  // Seguro Columna: 1.5%
  ...
  if (monto <= 75000) {
    // 0.90% trámite administrativo
    gastosAdmin = monto * 0.009;
  } 
  else if (monto > 75000 && monto <= 200000) {
    // 0.90% honorarios + seguro
    gastosAdmin = monto * 0.009;
  } 
  else if (monto > 200000 && monto <= 400000) {
    // Honorarios: 0.40%
    let honorarios = monto * 0.004;

    // Seguro: 0.50% sobre base fija de Q200,000
    let seguro = 200000 * 0.005;

    gastosAdmin = honorarios + seguro;
  } 
  else if (monto > 400000) {
    // Honorarios: 0.35%
    let honorarios = monto * 0.0035;

    // Seguro: 0.55% sobre base fija de Q200,000
    let seguro = 200000 * 0.0055;
  ...
  if (tipo === 'hipotecario') {
    ...
    // Solo validar cantidad de escrituras si monto > 400,000
    let escriturasHipotecario = 1; // Por defecto 1 escritura
    if (monto > 1000) {
      ...
      escriturasHipotecario = numEscriturasHipotecario;
    }

    pignoracion = monto * 0.01;
    infoAdicional.push(`Tipo escritura: Registrada`);
    infoAdicional.push(`Escrituras utilizadas: ${escriturasHipotecario}`);
    ...
    if (monto >= 1000 && monto <= 50000) {
      ...
      if (ubicacionAvaluo.value === 'solola') {
        ...
        avaluo = 1000 * escriturasHipotecario;
        infoAdicional.push(`Ubicacion avaluo: Solola (Q 1,000.00 x ${escriturasHipotecario} escritura${escriturasHipotecario > 1 ? 's' : ''} = Q ${formatoQuetzales(avaluo)})`);
      } else {
        // Q1,500 por cada escritura fuera de Solola
        avaluo = 1500 * escriturasHipotecario;
        infoAdicional.push(`Ubicacion avaluo: Fuera de Solola (Q 1,500.00 x ${escriturasHipotecario} escritura${escriturasHipotecario > 1 ? 's' : ''} = Q ${formatoQuetzales(avaluo)})`);
      }
    }
    ...
    if (escriturasHipotecario > 1) {
      const escriturasAdicionales = escriturasHipotecario - 1;
...
if (tipo === 'derechos_posesorios') {
  ...
  // Solo validar cantidad de escrituras si monto > 400,000
  let escriturasDP = 1; // Por defecto 1 escritura
  if (monto > 400000) {
    if (!cantidadEscrituras.value || numEscrituras < 1) {
      mostrarAlerta('error', 'Error de validacion', 'Por favor ingrese la cantidad de escrituras utilizadas.');
      return;
    }
    escriturasDP = numEscrituras;
  }

  pignoracion = monto * 0.01;
  infoAdicional.push(`Tipo escritura: Publica`);
  infoAdicional.push(`Escrituras utilizadas: ${escriturasDP}`);

  // =============================
  // AVALUO DERECHOS POSESORIOS
  // =============================
  if (monto >= 1000 && monto <= 50000) {
    avaluo = 150;
  } 
    ...
    if (!ubicacionDP.value) {
      mostrarAlerta('error', 'Error de validacion', 'Para creditos mayores a Q 400,000.00, debe seleccionar si esta en Solola o fuera.');
      return;
    }

    if (ubicacionDP.value === 'solola') {
      // Q1,000 por cada escritura en Solola
      avaluo = 1000 * escriturasDP;
      infoAdicional.push(`Ubicacion avaluo: Solola (Q 1,000.00 x ${escriturasDP} escritura${escriturasDP > 1 ? 's' : ''} = Q ${formatoQuetzales(avaluo)})`);
    } else {
      // Q1,500 por cada escritura fuera de Solola
      avaluo = 1500 * escriturasDP;
      infoAdicional.push(`Ubicacion avaluo: Fuera de Solola (Q 1,500.00 x ${escriturasDP} escritura${escriturasDP > 1 ? 's' : ''} = Q ${formatoQuetzales(avaluo)})`);
    }
  }
  ...
  // 1.5% gastos admin + 1% pignoracion ahorro + seguro (variable)
  // =============================
